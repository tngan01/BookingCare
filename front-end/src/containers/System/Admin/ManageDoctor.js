...
class ManageDoctor extends Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    // more-infor-extra
    if (prevProps.allRequiredDataInfor !== this.props.allRequiredDataInfor) {
      let { resPrice, resPayment, resProvince } =
        this.props.allRequiredDataInfor;
    ...
    // khi thay doi language thi build lai
  ...
  handleChangeSelect = async (selectedOption) => {
    ...
    // lay list tat ca data -> tim doi tuong dang duoc chon de hien thi
    let { listPrice, listPayment, listProvince } = this.state;
    let res = await getDetailInforDoctor(selectedOption.value);
    if (res && res.errCode === 0 && res.data && res.data.Markdown) {
      let markdown = res.data.Markdown;

      // hien thong tin da co
      let addressClinic = "",
        nameClinic = "",
        note = "",
        priceId = "",
  ...
  render() {
    let { hasOldData } = this.state;
    console.log("valueVi", this.state);
    return (
      <React.Fragment>
...
const mapStateToProps = (state) => {
  return {
    ...
    allRequiredDataInfor: state.admin.allRequiredDataInfor,
    // luu thong tin cho redux -> map qua react
  };
};
